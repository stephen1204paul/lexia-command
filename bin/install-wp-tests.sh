...
if [[ $WP_VERSION =~ ^[0-9]+\.[0-9]+\-(beta|RC)[0-9]+$ ]]; then
	...
	if [[ $WP_VERSION =~ [0-9]+\.[0-9]+\.[0] ]]; then
		...
		WP_TESTS_TAG="tags/${WP_VERSION%\.[0-9]}"
	else
	...
	GREP_VERSION_RESULT=$?
	if [[ $GREP_VERSION_RESULT -eq 0 ]]; then
		WORDPRESS_VERSION=$(grep -o '"version":"[^"]*"' /tmp/wp-latest.json | sed 's/"version":"\(.*\)"/\1/')
		if [[ $WORDPRESS_VERSION =~ [0-9]+\.[0-9]+\.[0] ]]; then
			WORDPRESS_VERSION=${WORDPRESS_VERSION%\.[0-9]}
		fi
...
install_wp() {
	...
	if [[ $WP_VERSION == 'nightly' || $WP_VERSION == 'trunk' ]]; then
		...
		git clone --depth=1 https://github.com/WordPress/WordPress.git $WP_CORE_DIR
	elif [[ $WP_VERSION =~ [0-9]+\.[0-9]+$ ]]; then
